...
public class Emmergency_Activity extends AppCompatActivity {
    private ListView listView;
    private Context context = this;
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activityemmergency);
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent =  new Intent(context,Others_Entry.class);
    ...
    private void setValues() {
        ...
        try{
            Cursor cursor = new Emmergency(context).selectAll();
            if (cursor.moveToFirst()){
                do {
                    name.add(cursor.getString(cursor.getColumnIndex(Constants.config.EMERGENCY_NAME)));
                    desc.add(cursor.getString(cursor.getColumnIndex(Constants.config.EMERGENCY_TYPE)));
                    contact.add(cursor.getString(cursor.getColumnIndex(Constants.config.PHONE_CONTACT)));
                    id_.add(cursor.getInt(cursor.getColumnIndex(Constants.config.EMERGENCY_ID)));
                    district_.add(cursor.getString(cursor.getColumnIndex(Constants.config.DISTRICT_NAME)));
                }while (cursor.moveToNext());
            }
            Other_Adapters adapters = new Other_Adapters(context,name,desc,contact, id_,district_,"emergency");
