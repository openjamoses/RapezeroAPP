    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent =  new Intent(context,Police_Entry.class);
    ...
    private void setValues() {
        ...
        try{
            ...
            if (cursor.moveToFirst()){
                do {
                    name.add(cursor.getString(cursor.getColumnIndex(Constants.config.POLICE_NAME)));
                    desc.add(cursor.getString(cursor.getColumnIndex(Constants.config.POLICE_TYPE)));
                    contact.add(cursor.getString(cursor.getColumnIndex(Constants.config.PHONE_CONTACT)));
                    id_.add(cursor.getInt(cursor.getColumnIndex(Constants.config.POLICE_ID)));
                    district_.add(cursor.getString(cursor.getColumnIndex(Constants.config.DISTRICT_NAME)));
                }while (cursor.moveToNext());
            }
            Other_Adapters adapters = new Other_Adapters(context,name,desc,contact,id_,district_,"police");
